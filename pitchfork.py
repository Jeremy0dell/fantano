...
class Review:
    ...
    def genres(self):
        ...
        if hasattr(self.soup.find(class_='genre-list'), 'children'):
            genres_node = self.soup.find(class_='genre-list')
            genres = [genre.get_text() for genre in genres_node]
            genres = ','.join(genres)
            return genres
        else:
            return ''
    ...
    def year(self):
        ...
        """
        year = self.soup.find(class_='single-album-tombstone__meta-year').get_text()[3:]
        return year
